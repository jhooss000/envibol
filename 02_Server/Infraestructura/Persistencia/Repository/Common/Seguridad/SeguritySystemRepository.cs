...
{
    public class SeguritySystemRepository : ISeguritySystemRepository
    {
        private readonly SegurityContext _context;
        private readonly IMapper _mapper;
        private readonly JWTSettings _JWTSettings;

        public SeguritySystemRepository(SegurityContext context, IMapper mapper, IOptions<JWTSettings> jWTSettings)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<SegUsuarioDto> ValidateUserSystemLoguin(string pUsuario, string pIpAddress)
        {
            var usuario = await Task.FromResult(_context.Usuario
                                .Where(f => f.LoginUsuario.ToUpper() == pUsuario.ToUpper()
                                        && f.IdcEstado == ((int)Helpers.ESTADO_CUENTA.ACTIVO).ToString())
                                .FirstOrDefault());
